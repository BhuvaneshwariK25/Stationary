...
public class MainVerticle extends AbstractVerticle {
    ...
    public void start() {
        ...
        router.post("/inventory").handler(this::addItems);
    ...
    private void addItems(RoutingContext context) {
        JsonArray items = context.getBodyAsJsonArray();
        for (int i = 0; i < items.size(); i++) {
            JsonObject item = items.getJsonObject(i);
            mongoClient.save("inventory", item, res -> {
                if (res.failed()) {
                    context.response().setStatusCode(500).end("Failed to add items: " + res.cause().getMessage());
                    return;
                }
            });
        }
        context.response().setStatusCode(201).end("Items added successfully");
    }
